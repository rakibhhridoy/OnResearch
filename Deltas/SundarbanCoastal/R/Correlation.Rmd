...
```{r}
getwd()
df <- read.csv("~/Research/MatureDelta/data/MatureDeltaDatalong.csv")
df
```

```{r}
names(df)
df <- df[, c("Bulk.Density", "Moisture", "Sand", "Silt", "Clay", "pH", "EC", "OM", "TN", "CEC")]
df
```
...
```{r}
write.csv(corr_metals, "~/Research/MatureDelta/data/CorrData.csv")
```
